...
namespace FlowGraph.Model
{
    ...
    public abstract class FlowNode : ISerializationCallbackReceiver
    {
        ...
        protected ValuePort<T> AddValueInput<T>(string name, Inject inject = null)
        {
            ValuePort<T> port = new ValuePort<T>(this, name, inject);
        ...
        {
            string name = null;
            var nameAttr = GetType().GetCustomAttributes(typeof(NameAttribute), true).FirstOrDefault() as NameAttribute;
            if (nameAttr != null)
                name = nameAttr.Name;
        ...
        public virtual string GetDisplayNameDesc()
        {
        ...
        protected ValuePort AddValueInput(string name, Type type, Inject inject = null)
        {
            ...
            return (ValuePort)m.Invoke(this, new object[] { name, inject });
        }
        ...
        protected void RegisterMethodPorts(MethodInfo method, bool igoreInstance, bool hasReturn, string returnOutputName = null)
        {
            ...
            if (!method.IsStatic && !igoreInstance)
            {
                ...
                var thisIn = AddValueInput(THIS, thisType, new Inject(thisType));
            ...
            foreach (var pInfo in method.GetParameters())
            {
                ...
                if (pInfo.IsDefined(typeof(HiddenAttribute), false))
                    flags |= ValuePortFlags.Hidden;

                var injectAttr = pInfo.GetCustomAttributes(typeof(InjectAttribute), false).FirstOrDefault() as InjectAttribute;
                if (injectAttr != null)
                {
                    if (injectAttr.Type == null && string.IsNullOrEmpty(injectAttr.Name))
                    {
                        inject = new Inject(pInfo.ParameterType);
                    }
                    else if (injectAttr.Type != null)
                        inject = new Inject(injectAttr.Type);
                    else if (injectAttr.Name != null)
                        inject = new Inject(injectAttr.Name);
                }

                if (pInfo.IsRef())
                {
                    port = AddValueInput(pInfo.Name, pInfo.ParameterType, inject);
                    port.Flags = flags;
        ...
        public static Inject GetDefaultInject(Type type)
        {
            Inject inject = null;
            if (type == typeof(GameObject))
                inject = new Inject(InjectValueType.GameObject);
            else if (type == typeof(ExecutionContext))
