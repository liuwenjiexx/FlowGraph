...
namespace FlowGraph
{
    ...
    internal static class Extensions
    {
        ...
        public static void LabelFit(this GUIStyle style, GUIContent content, int maxWidth)
        {
            ...
            rect.width = maxWidth;
            LabelFit(style, rect, content);

        }

        public static void LabelFit(this GUIStyle style, Rect rect, GUIContent content)
        {
            var size = style.CalcSize(content);
            float maxWidth = rect.width;
            float maxHeight = rect.height;
            maxWidth -= style.margin.horizontal;
            maxHeight -= style.margin.vertical;

            if (size.x > maxWidth || size.y > maxHeight)
            {
                GUIStyle s = new GUIStyle(style);

                while ((size.x > maxWidth || size.y > maxHeight) && s.fontSize > 8)
                {
                    s.fontSize--;
